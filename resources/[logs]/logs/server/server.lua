...
exports('discord', function(message, id, id2, color, channel)
    ...
    if id2 ~= 0 then
        ...
        if Config.postal then
            _postal2 = "\n**Nearest Postal:** " .. postal2 .. ""
        else
            _postal2 = ""
        end
        if Config.discordID then
            if ids2.discord ~= "" then
                _discordID2 = "\n**Discord ID:** <@" .. ids2.discord:gsub("discord:", "") .. ">"
            else
                _discordID2 = "\n**Discord ID:** N/A"
            end
        else
        ...
        if Config.steamURL then
            if ids2.steam ~= "" then
                _steamURL2 = "\nhttps://steamcommunity.com/profiles/" .. tonumber(ids2.steam:gsub("steam:", ""), 16) .. ""
            else
                _steamURL2 = "\n**Steam URL:** N/A"
            end
        else
        ...
        if Config.license then
            if ids2.license ~= "" then
                _license2 = "\n**License:** " .. ids2.license
            else
                _license2 = "\n**License :** N/A"
            end
        else
...
function discordLog(message, color, channel)
    if Config.AllLogs then
        PerformHttpRequest(Config.Server.webhooks["all"], function(err, text, headers)
            ...
            embeds = {{
                ...
                ["footer"] = {
                    ["text"] = os.date("%x %X %p")
                    -- ["icon_url"] = "https://cdn.discordapp.com/attachments/1098847427995369593/1105797235570130944/server.png"
                }
            }},
    ...
    PerformHttpRequest(Config.Server.webhooks[channel], function(err, text, headers)
    end, 'POST', json.encode({
        username = Config.username,
        embeds = {{
            ["color"] = color,
...
--         PerformHttpRequest('https://raw.githubusercontent.com/JokeDevil/JD_logs/master/version.json', function(code, res, headers)
--             if code == 200 then
