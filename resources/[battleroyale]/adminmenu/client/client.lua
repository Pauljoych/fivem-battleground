local ESX = exports['vd_core']:getSharedObject()
local colors = { {
    name = "Black",
    colorindex = 0
}, {
...
local menu = MenuV:CreateMenu(" ", 'Developed By Narchy-', 'topright', 102, 0, 204, 'size-150', 'template1', 'menuv',
    'mainmenu')
...
local LoadOnlinePlayersEach = function()
    ...
    if IsPlayerAllowed('TSAdmin.OnlinePlyOptions.SendMessage') then
        ...
        local onlineplayers_each_sendm = onlineplayers_each:AddButton({
            ...
            false,
            select = function(i)
                local input = lib.inputDialog('Admin Menu', { 'Message' })
    ...
    RegisterNetEvent('ts-adminmenu:client:setgrade', function(data)
        ...
        local input = lib.inputDialog('Admin Menu', { "Max Grade: " .. max })
    ...
    --     local SelectItems = function()
    --         local input = lib.inputDialog('Admin Menu', { "Item Code" })
    ...
    --         local input = lib.inputDialog('Admin Menu', { "Player ID", "Item Name" })
    ...
    if IsPlayerAllowed('TSAdmin.OnlinePlyOptions.GiveMoney') then
        ...
        local onlineplayers_each_givem = onlineplayers_each:AddButton({
            ...
            false,
            select = function(i)
                local input = lib.inputDialog('Add Account Money - Admin', { "Account Name (money, bank, black_money)", "Amount" })
    ...
    if IsPlayerAllowed('TSAdmin.OnlinePlyOptions.RemoveMoney') then
        ...
        local onlineplayers_each_removem = onlineplayers_each:AddButton({
            ...
            false,
            select = function(i)
                local input = lib.inputDialog('Remove Account Money - Admin', { "Account Name (money, bank, black_money)", "Amount" })
    ...
    if IsPlayerAllowed('TSAdmin.OnlinePlyOptions.License') then
        ...
        local onlineplayers_each_lic = onlineplayers_each:AddButton({
            ...
            false,
            select = function(i)
                local input = lib.inputDialog('Admin Menu - License Manager', { "License Name" })
...
local LoadVehicleOptions = function()
    ...
    if IsPlayerAllowed('TSAdmin.VehicleRelatedOptions.DeleteVehicle') then
        ...
        local vehicleoptions_vehicleop_deleter = vehicleoptions:AddButton({
            ...
            select = function(i)
                local input = lib.inputDialog('Admin Menu - Delete Vehicle In Radius', { "Radius" })
    ...
    if IsPlayerAllowed('TSAdmin.VehicleRelatedOptions.Spawner') then
        ...
        CreateThread(function()
            ...
            vehicleoptions_vehiclespawn:AddButton({
                ...
                false,
                select = function(i)
                    local input = lib.inputDialog('Admin Menu - Vehicle Spawner', { "Spawn Code" })
    ...
    if IsPlayerAllowed('TSAdmin.VehicleRelatedOptions.NumberPlate') then
        ...
        local vehicleoptions_vehicleop_setplate = vehicleoptions_vehicleop:AddButton({
            ...
            false,
            select = function(i)
                local input = lib.inputDialog('Admin Menu - Plate Changer', { "Plate" })
...
local LoadPlayerOptions = function()
        ...
        local playeroptions_reviveid = playeroptions:AddButton({
            ...
            select = function(i)
                ...
                if veh ~= 0 then
                    ...
                    local input = lib.inputDialog('Admin Menu - Give Car', { "Player ID" })
    ...
    if IsPlayerAllowed('TSAdmin.PlayerOptions.DeleteCar') then
        ...
        local playeroptions_reviveid = playeroptions:AddButton({
            ...
            false,
            select = function(i)
                local input = lib.inputDialog('Admin Menu - Delete Car', { "Plate" })
                    ...
                    if dist < 450 then
                        ...
                        if v ~= PlayerId() then
                            ...
                            if GetVehiclePedIsUsing(PlayerPedId()) then
                                SetBlipSprite(bliptony, 225)
                                SetBlipScale(bliptony, 1.0)
                                SetBlipAlpha(bliptony, 255)
                                SetBlipColour(bliptony, 13)
...
RegisterCommand('tsadmin', function()
    if IsPlayerAllowed('TSAdmin.admin') then
        ...
        lib.notify({
            title = 'Admin Menu',
            description = 'Koe udu Admin!',
            type = 'error'
        })
    end
end)

RegisterKeyMapping('tsadmin', 'VD Adminmenu', 'keyboard', 'f11')
...
RegisterNetEvent('ts-adminmenu:client:ChangePed', function()
    Wait(500)
    local input = lib.inputDialog('Admin Menu - Ped Changer', { "Player ID", "Model Name" })
...
RegisterNetEvent('ts-adminmenu:client:GiveItem', function(data)
    ...
    Wait(500)
    local input = lib.inputDialog('Admin Menu - Give Item', {"Count"})
...
RegisterNetEvent('ts-adminmenu:client:RemoveItemCount', function(data)
    ...
    local item = data.item
    Wait(500)
    local input = lib.inputDialog('Admin Menu - Remove Item', { "Count"})
