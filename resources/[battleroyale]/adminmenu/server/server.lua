...
lib.callback.register('ts-adminmenu:getAuthorization', function(source, obj)
    ...
    local allowed = false
    if IsPlayerAllowed(source,obj) or xPlayer and (group == 'superadmin') then
        allowed = true
    end
    return allowed
end)
...
RegisterNetEvent('ts-adminmenu:server:playerDied', function(data)
    ...
    if ply2 then
        msg = '~o~' .. xPlayer.getName() .. '~s~ Dibunuh Oleh ~y~' .. yPlayer.getName() .. '~s~ Death Cause: ' ..
                  reason
    else
        msg = '~o~' .. xPlayer.getName() .. '~s~  Alasan: ' .. reason
    end
    for k, v in pairs(Players) do
    ...
    for k, v in pairs(Players) do
        if CheckAllowed(v.source, 'TSAdmin.MiscSettings.JoinQNotif') then
            TriggerClientEvent('ts-adminmenu:client:JoinQ', v.playerId,
                '~o~' .. GetPlayerName(src) .. '~s~ Keluar Server Reason: ~y~' .. reason .. '~s~')
        end
    end
end)
